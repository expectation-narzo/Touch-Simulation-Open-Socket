...
void *AimBotThread() {
    ...
    while (1) {
       ...
       // t.SetTouchFingerNum(FingerMax);
        ...
        pvm(getA(Aim.objAddr + 0x154) + 0x1a0, &running, sizeof(running));
...
{
		return Read<MinimalViewInfo>(camera + 0x360 + 0x10);
}
...
int main()
{
	...
	//./ue4dumper25 --sdku --package com.pubg.krmobile --gname 0x852082C --guobj 0x86de650
	uintptr_t gname = getA(getA(base + 0x852082C) + 0x80);
	if (strstr(version, "com.tencent.ig"))
		gname = getA(getA(base + 0x852082C) + 0x80);
	...
	while (isBeta || (receive((void*)&request) > 0)) {
		...
		Ulevel ulevel;
		kaddr actorArr = DecryptActorsArray(enAddrPntr - 0x70, 0x70, 0x334);
		...
		firing = 0;
		ads = 0;
		aimLocation.X = 0 ,aimLocation.Y = 0;
		
		kaddr gWorld = getGWorld(base);
		if(!gWorld) continue;
		
		MinimalViewInfo POV = MinimalViewInfo();
		kaddr PlayerController = getA(getA(getA(gWorld + 0x24) + 0x60) + 0x20);
		if(PlayerController) {
		    kaddr camm = getA(PlayerController + 0x350);
		    POV = getPOV(camm);
		    //printf("FOV : %f\n", POV.FOV);
		}
		...
		for (int i = 0; i < ulevel.size; i++) {
			...
			if (strstr(name, "BP_PlayerPawn")) {//Player
				...
				*/
				...
				pvm(pBase + 0x9ac, healthbuff, sizeof(healthbuff));
				if (healthbuff[1] > 600.0f || healthbuff[1] < 30.0f || healthbuff[0]>healthbuff[1] || healthbuff[0] < 0.0f)
					continue;
				PlayerData* data = &response.Players[response.PlayerCount];
				data->Health = healthbuff[0] / healthbuff[1] * 100;

				data->TeamID = getI(pBase + 0x694);

				//me
				uintptr_t me = getI(pBase + 0x100);
				if (me == 258) {
					if (isBeta)
						printf("\nMe(%d): %lX ", data->TeamID, pBase);
					if (aimbot) {
						// yawPitch = getA(pBase + 0x2e4) + 0x2e0;
						if (aimWhen == 3 || aimWhen == 2)
							pvm(pBase + 0xfb8, &firing, 1);
						if (aimWhen == 3 || aimWhen == 1)
							pvm(pBase + 0xb7d, &ads, 1);
					}
				...
				pvm(getA(pBase + 0x5564) + 0x150, &exyz, sizeof(exyz));

				data->HeadLocation = WorldToScreen(exyz, POV, width, height);

				data->Distance = getDistance(xyz, exyz);
				if (data->Distance > 600.0f)
					continue;
				pvm(pBase + 0x710, &data->isBot, sizeof(data->isBot));
				...
				if (data->HeadLocation.Z != 1.0f && data->HeadLocation.X < width + 100 && data->HeadLocation.X > -50) {
					...
					if (!data->isBot) {
						strcpy(data->PlayerNameByte, getNameByte(getA(pBase + 0x660)));
					...
					if (centerDist < aimRadius) {
						...
						if (nearest > centerDist || nearest < 0) {
							...
							if (aimFor == 1){
								...
								uintptr_t boneAddr = getA(pBase + 0x330);
								struct D3DMatrix baseMatrix = getOMatrix(boneAddr + 0x140);
								boneAddr = getA(boneAddr + 0x634);
				...
				pvm(getA(pBase + 0x154) + 0x150, &exyz, sizeof(exyz));

				data->Location = WorldToScreen(exyz, POV, width, height);
...
uintptr_t getMatrix(uintptr_t base) {
    return getA(getA(base + 0x8841698) + 0x7c);
}

uintptr_t getEntityAddr(uintptr_t base)
{
	if (strstr(version, "com.tencent.ig"))
		return getA(getA(getA(base + 0x8858574) + 0x3c) + 0x20) + 0x70;
	return getA(getA(getA(base + 0x8858574) + 0x3c) + 0x20) + 0x70;
}
...
{
	if (strstr(version, "com.tencent.ig"))
		return getA(getA(base + 0x8858574) + 0x3c);
	return getA(getA(base + 0x8858574) + 0x3c);
}
