...
extern "C"
{
	...
	int httppost(const char *hostname, const char *url, const char *cs, char **run);	  //POST方法
...
int httppost(const char *hostname, const char *url, const char *cs, char **run)
{
	// sock句柄
	int sockfd;
...
//const char base[] = {"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890+/"};
static char base[65] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890+/";
...
// 校验函数
int Check(char *jm, const char *km, const char *imei);	// 第一个不用const，这样如果编译参数是一个常量的话会出警告

/* c验证开启 */

const static char *host = "api.yiyanzheng.cn";
const static char *appid = "12826";
const static char *key = "666";
const static int num = 666;
const static char bmj[65] = "UGsXSLPoANZbhB9OVJ7lM0pDFHmx+nRjvwf2E3Iqgz5TY61duQy/ekriK4C8catW";
const static char *zdykami = "zy159665062989410863";
const static char *zdyimei = "96f876cc92e44a67";

int Check(char *jm, const char *km, const char *imei)
{
	char *dl;
	if ((dl = strstrstr(jm, "登录验证:", ";", true)) == NULL || strcmp(dl, "成功") != 0)
		return -1;
	char *md5 = strstrstr(jm, "识别码:", ";", false);
	char *kmid = strstrstr(jm, "卡密ID:", ";", false);
	char yz[128];
	time_t t = time(NULL);
	...
	sprintf(yz, "%d%s%s%s%s", atoi(kmid) + num, key, km, kmid, sj);

	MD5_CTX md5c;
	MD5Init(&md5c);
	int i;
	unsigned char decrypt[16];
	// C++此处必须强转无符号，别问，问就是垃圾（鄙视）
